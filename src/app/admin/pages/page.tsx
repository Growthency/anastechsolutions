...
import {
  Plus, Pencil, Trash2, Loader2, Link2,
...
function countInternalLinks(html: string): number {
  if (!html) return 0
  const matches = html.match(/<a\s[^>]*href=["'](\/[^"']*|https?:\/\/anastechsolutions\.com[^"']*)["'][^>]*>/gi)
...
export default function AdminPagesPage() {
  ...
  const handleDelete = async (id: number) => {
    const ok = await showConfirm('Delete Page', 'Are you sure you want to delete this page? This action cannot be undone.', 'danger')
    if (!ok) return
    setDeleting(id)
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: dark ? '#64748b' : '#94a3b8' }}>{total} total pages</p>
        </div>
        <Link
      ...
      <div className="rounded-2xl overflow-hidden" style={{ border: `1px solid ${dark ? 'rgba(255,255,255,0.06)' : '#e2e8f0'}`, background: dark ? 'rgba(255,255,255,0.02)' : '#fff' }}>
        {loading ? (
          ...
          <div className="text-center py-20">
            <p className="text-[13px]" style={{ color: dark ? '#64748b' : '#94a3b8' }}>No pages found</p>
            <Link href="/admin/pages/new" className="text-[13px] mt-2 inline-block hover:underline" style={{ color: '#0F75BC' }}>
              ...
              {filtered.map(post => (
                <tr key={post.id} className="transition-colors" style={{ borderBottom: `1px solid ${dark ? 'rgba(255,255,255,0.04)' : '#f1f5f9'}` }}>
                  ...
                  <td className="px-4 py-3.5 hidden lg:table-cell">
                    {post.is_premium ? (
                      <span className="inline-flex items-center gap-1 text-[11px] px-2 py-0.5 rounded-lg bg-amber-500/10 text-amber-500 ring-1 ring-amber-500/20 font-medium">
                        <Lock className="w-3 h-3" /> Premium
                      </span>
                    ) : (
                      <span className="inline-flex items-center gap-1 text-[11px] px-2 py-0.5 rounded-lg font-medium" style={{ background: 'rgba(15,117,188,0.1)', color: '#0F75BC' }}>
                  ...
                  <td className="px-4 py-3.5 hidden md:table-cell">
                    <span className="text-[12px] font-medium" style={{ color: dark ? '#94a3b8' : '#64748b' }}>{(post.views || 0).toLocaleString()}</span>
                  </td>
                  <td className="px-4 py-3.5 hidden md:table-cell">
                    {(() => {
            ...
            {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => {
              ...
              return (
                <button
                  ...
                  style={p === page
                    ? { background: '#0F75BC', borderColor: '#0F75BC', boxShadow: '0 6px 16px rgba(15,117,188,0.25)' }
                    : { borderColor: dark ? 'rgba(255,255,255,0.06)' : '#e2e8f0', color: dark ? '#94a3b8' : '#64748b' }
                  }
                >
                  {p}
                </button>
